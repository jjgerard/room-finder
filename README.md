...
Two pages. The first has four tabs, in this order:

| Tab | What it shows |
|---|---|
| **Autumn 2026** — as it stands | For reference; the rebuild covers Spring only. |
| **Spring 2026** — as it stands | The problem: bookings marked *split across rooms* are one class occupying two or three. |
| **Spring 2026** — rebuilt | The result. The hard rules are re-checked **in the browser on load**, so the headline is verified rather than asserted. |
| **Method** | How it was built and where the numbers are soft. |

**Explore moves** is a separate page, last in the nav: pick a class, see every slot it could
legally move to, and for the ones it can't, exactly what is in the way.

Search is **by programme first**, then module, then room — a cohort is what people actually
ask about. Autumn and current-Spring are shown one row per class-room booking, so a class
split across rooms appears once per room and the splitting is visible.

`.github/workflows/ci.yml` does not deploy; Pages serves the folder directly. It runs the
