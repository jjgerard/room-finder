...
Live at **https://jjgerard.github.io/room-finder/** once Pages is switched on:
**Settings → Pages → Source: "GitHub Actions"**. That is a one-time click; after it,
`.github/workflows/pages.yml` redeploys on every push that touches `docs/` or
`timetable/`. The workflow runs the tests first, and fails the build if
`docs/assets/` has drifted from `timetable/lib/` — the copies are generated by
`node timetable/export.js`, and a hand-edited copy would make the site and the solver
